...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,boolean evict) {
        ...
        int n = tab.length;
        int i; // key要put进数组的下标索引
        // 先判断数组是否为空，如果为空，进行初始化
        if (tab == null || n == 0) {
            tab = resize();
            n = tab.length;
        }
        // 计算key到put到数组的下标位置（数组长度-1跟hash做与运算，得到的结果就是0到数组长度之间的整数，能达到取模运算的效果，性能要比取模运算好）
        i = (n - 1) & hash;
        // 获取到数组中当前位置上的Node元素，（有可能是null,有可能是链表，也有可能是红黑树）
        p = tab[i];
        ...
        if (p == null) {
            ...
            // 判断数组当前位置的元素是不是红黑树，
                ...
                for (int binCount = 0; ; ++binCount) {
                    // e赋值为当前位置元素的下一个元素
                    e = p.next;
                    // 循环，直到链表的末尾节点，即该节点的next=null
                    if (e == null) {
                        // 如果下一个元素为null，说明已经走到了链表的末端，
                        // **将本次要put的值添加到链表的末尾**
                        p.next = newNode(hash, key, value, null);
                        // 判断如果数组该位置上的链表长度，如果超过8（包含8），则将链表转为红黑树
                        if (binCount >= TREEIFY_THRESHOLD - 1) // -1 for 1st
                            // 链表转为红黑树
                            treeifyBin(tab, hash);
                        break;
                    }
                    // 如果在链表中某个节点的key与当前要put的key相同了，则退出循环，此时key相同的节点e也已找到
                    if (e.hash == hash && ((k = e.key) == key || (key != null && key.equals(k)))){
                        break;
                    }
                    // 如果循环中没找到key相同的节点，且还没到链表末端，则继续循环
                    // 将下一个节点设置为当前节点，继续循环
                    p = e;
                }
            }
            // e!=null,说明要put的key存在重复，且重复的节点就是e
            if (e != null) { // existing mapping for key
                // 记录该key原来的值
                V oldValue = e.value;
                // 如果配置的参数【仅仅允许key不存在时插入】为flase,或者该key原来的值为null时，替换当前key对应的value
                if (!onlyIfAbsent || oldValue == null) {
                    // 更新当前已存在节点e的value为本次要put的value
                    e.value = value;
                }
                // 更新值后允许子类去执行自定义的功能，扩展类LinkedHashMap在用
                afterNodeAccess(e);
                // 返回原始值
                return oldValue;
            }
        ...
        ++modCount;
        // map的容量先自增，再判断是否需要扩容
        // threshold = capcity * loadFactory(默认 12 = 16 * 3/4)
        if (++size > threshold){
            // 扩容(初始化也调用resize方法)
            resize();
        }
